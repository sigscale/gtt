...
%% @hidden
start3(TopSup, EpFsmSup, AsFsmSup, [AS | T]) ->
	F = fun() -> mnesia:read(gtt_as, AS, read) end,
	start4(TopSup, EpFsmSup, AsFsmSup, mnesia:transaction(F), T);
start3(TopSup, EpFsmSup, _, []) ->
	F = fun() -> mnesia:all_keys(gtt_ep) end,
	case mnesia:transaction(F) of
		{atomic, EndPoints} ->
			start5(TopSup, EpFsmSup, EndPoints);
		{aborted, Reason} ->
			{error, Reason}
	end.
%% @hidden
start4(TopSup, EpFsmSup, AsFsmSup, {atomic, [#gtt_as{name = Name, role = Role,
...
start4(_, _, _, {aborted, Reason}, _) ->
	{error, Reason}.
%% @hidden
start5(TopSup, EpFsmSup, [Name | T]) ->
	StartMod = gtt_ep_fsm,
	StartArgs = [Name],
	StartOpts = [],
